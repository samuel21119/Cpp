...
/*
    使用方式：
            1. 先宣告BigNumber Class的一個Class (如 BigNumber var;)
            2. 若要輸入數字，請用Scan()函式      (如 var.Scan();)
            3. 若要運算數字，請使用Calc()函式     (如 var.Calc();)
            4. 若要輸出結果，請用Print()函式或Print('C');     (如 var.Print();)
            5. 若要輸出第一個輸入的數字，請用Print('A'); ，若需要輸出第二的輸入值，請用Print('B');  (如 var.Print('A');)
            6. 程式結束時，請使用Free()函式釋放所有記憶體  (如 var.Free();)
*/
