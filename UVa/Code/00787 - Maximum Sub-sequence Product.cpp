/*************************************************************************
  > File Name: 00787 - Maximum Sub-sequence Product.cpp
  > Author: Samuel
  > Mail: enminghuang21119@gmail.com
  > Created Time: Fri Feb  1 11:51:06 2019
*************************************************************************/
...
int main() {
    ...
    while (getline(cin, s)) {
        ...
        for (int i = 1; i < n; i++) {
            Number t = dp[i][0] + 0;
            if (dp[i][0] > 0) {
                dp[i][0] = max(dp[i][0] + 0, dp[i - 1][0] * t);
                dp[i][1] = min(dp[i][0] + 0, dp[i - 1][1] * t);
            }
            else {
                dp[i][0] = max(dp[i][0] + 0, dp[i - 1][1] * t);
                dp[i][1] = min(dp[i][1] + 0, dp[i - 1][0] * t);
            }
            //cout << dp[i][0] << ' ' << dp[i][1] << endl;
            ans = max(ans + 0, dp[i][0] + 0);
        }
        cout << ans << '\n';
    }
    return 0;
